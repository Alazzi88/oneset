...
// مكون بطاقة العقار مع مودال الصور
function PropertyCard({ item }: { item: Listing }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalIndex, setModalIndex] = useState(0);

  const openModal = (i: number) => { setModalIndex(i); setIsModalOpen(true); };
  const showPrev = (e: React.MouseEvent) => { e.stopPropagation(); setModalIndex(p => (p - 1 + item.imageUrls.length) % item.imageUrls.length); };
  const showNext = (e: React.MouseEvent) => { e.stopPropagation(); setModalIndex(p => (p + 1) % item.imageUrls.length); };
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center" onClick={() => setIsModalOpen(false)}>
          <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} transition={{ duration: 0.3 }} className="relative w-full max-w-3xl max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <img src={item.imageUrls[modalIndex]} alt={`${item.title} - مكبرة`} loading="lazy" className="object-contain rounded-lg w-full h-auto" />
            <button onClick={showPrev} className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-white bg-opacity-70 rounded-full p-2 hover:bg-opacity-100"><ChevronLeft size={24} /></button>
            <button onClick={showNext} className="absolute top-1/2 right-4 transform -translate-y-1/2 bg-white bg-opacity-70 rounded-full p-2 hover:bg-opacity-100"><ChevronRight size={24} /></button>
